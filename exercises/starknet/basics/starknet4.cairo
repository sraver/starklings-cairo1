...
mod LizInventory {
    ...
    impl LizInventoryImpl of super::ILizInventory<ContractState> {
        fn add_stock(ref self: ContractState, product: felt252, new_stock: u32) {
            ...
            self.only_owner();
            let mut current_stock = self.inventory.read(product);
            current_stock += new_stock;
            self.inventory.write(product, current_stock);
        }
        ...
        fn purchase(ref self: ContractState, product: felt252, quantity: u32) {
            ...
            let mut current_quantity = self.inventory.read(product);
            assert(quantity <= current_quantity, 'Not enough stock');
            current_quantity -= quantity;
            self.inventory.write(product, current_quantity);
        }
